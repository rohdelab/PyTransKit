...
filename = '/Users/natasha/Desktop/Research/Codes/TBM_code/'; %input filename here 
cd(filename);
...
imagefolder = (strcat(filename,'/ERICHmatfiles/')); %folder with nifti images, can be saved as '.nii.gz' or '.nii'
niifolder = strcat(filename, '/ERICHniifiles/')
%matfolder = (strcat(filename,'/ERICHmatfiles/')); %folder to save matfiles  
%addpath(strcat(filename, '/Load_nii/'))
a = dir(fullfile(imagefolder,'*.mat')); %can also read in '.nii.gz', just change the suffix
n = numel(a);
numfiles = length(a); 
for i=1:numfiles
    ...
    file1 = load(file)
    image = file1.image;
    nii = make_nii(image); %calls the load_nii function
    [filepath,name,ext] = fileparts(file);
    newname = strsplit(name, '.'); %stores the name of the original file
    save_nii(nii, strcat(niifolder, name, '.nii'))
    %image = nii.img;
...
%% translate images to center of template without scaling
...
%template = load(strcat(filename,'ERICHtemplate/translatedtemplate.mat')) %load the template image
template = translatedtemplate;
%template = thresh_template(template); %threshold template to create sharp edges
templatebin = imbinarize(template); %binarize template 
...
for i=1:numfiles1
    ...
    a = find(stats2.Volume == max(stats2.Volume))
    centroid = stats2.Centroid;
    centroidi(i,:) = centroid(a,:)
    translationb = [centroidi(i,1)-centroidt(1), centroidi(i,2)-centroidt(2), centroidi(i,3)-centroidt(3)];%generate translation vector to center of template, with reference to the template
...
matfolder = (strcat(filename,'/ERICHtranslatednew/redo/')) %folder where you saved the matfiles
transportfolder = (strcat(filename,'/ERICHtranslatedtransportredo/')); %folder to save the transport maps 
...
for i=1:numfiles1
    ...
    I0 = template; %I0 is the Euclidean mean template. You are computing the optimal transport distance from the template to each image.
    source = load(file1)
    translationi = source.translationi;
    I1 = source.translatedimage; %I1 is the segmented image 
    results = multVOT(double(I0),double(I1),50,10^6,1,4,0.25,0.1); %Apply 3D linear optimal transportation.
    save(strcat(transportfolder, newname{1}, '.mat'), "results", "translationi") %saving the results for each image 
end
...
for i = 1:length(n_std)
    ...
    disp = A_mean + n*sigmatrain*V1'; %if creating template image disp = A_mean, remove n*sigmatrai*V1'
