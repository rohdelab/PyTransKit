...
authors:
  - name: Hongyu Kan
    orcid: 0009-0001-2806-4888
    affiliation: '1'
  - name: Kristofor Pas
    orcid: 0000-0003-3751-0867
    affiliation: '1'
  - name: Ivan Medri
    orcid: 0000-0003-2419-2193
    affiliation: '1'
  - name: Naqib Sad Pathan
    orcid: 0000-0002-1571-8221
    affiliation: '1'
  - name: Natasha Ironside
    orcid: 0009-0006-3130-5829
    affiliation: '1'
  - name: Shinjini Kundu
    orcid: 0000-0003-4930-1851
    affiliation: '2'
...
In this work, we present 3D-TBM, an open source Python-based tool to enable TBM to be applied to 3D medical imaging. 3D-TBM can be used to analyze the relationship between morphological features and given variables. It consists of four steps: data pre-processing; embedding 3D images into the transport domain; using a statistical model for variable analysis, classification, and regression; and visualizing the analysis results. Each module is encapsulated within a dedicated interface and users can flexibly adjust the parameter settings according to their specific needs. 

# State of the field

Although other medical imaging tools offer convenient functionalities for morphological analysis - such as registration, segmentation, and even neural network training - they do not provide tools to perform transport-based morphometry. Advanced Normalization Tools (ANTs) is a C++-based command-line library for high-dimensional biomedical image registration and analysis, enabling the statistical exploration, visualization, and integration of large-scale imaging data across modalities, species, and organ systems [@avants2009advanced]. Extensions of ANTs include ANTsR and ANTsPy, as well as ANTsX, which integrates deep learning methods into the ANTs framework [@tustison2021antsx;@tustison2024antsx]. MONAI is an open-source, community-driven PyTorch-based framework that extends PyTorch for medical imaging, providing specialized architectures, transformations, and utilities to facilitate the development and deployment of healthcare AI models [@cardoso2022monai].
FreeSurfer is a comprehensive neuroimaging analysis suite that quantifies functional, connectional, and structural brain properties, evolving from cortical surface modeling to the automated reconstruction of most macroscopically visible brain structures from T1-weighted images [@fischl2012freesurfer;@dale1999cortical]. SPM is a software package for analyzing brain imaging data sequences, including cross-sectional datasets from multiple cohorts and longitudinal time-series from individual subjects [@friston1994statistical;@penny2011statistical]. AFNI (Analysis of Functional NeuroImages) is a comprehensive software suite composed of C, Python, and R programs, along with shell scripts, designed primarily for analyzing and visualizing various MRI modalities, including anatomical, functional (fMRI), and diffusion-weighted (DW) data[@cox1996afni;@cox1997software]. However, none of these software packages currently provide dedicated tools for TBM analysis.

# Software design

![The workflow of Transport-Based Morphometry (TBM). TBM can be used to analyze the relationship between the morphological features of medical images and given variables. The plot on the right shows how an independent variable (age) can be correlated with a TBM index score, which can be associated with a typical/average white matter morphology (dependent variable). The software automatically identifies associations between aging and white matter changes. The visualized features include white matter tissue atrophy, progressive enlargement of the ventricles and prominence of the sulci, representing volume loss. The advantage of this method is that its findings can be used to generate hypotheses for underlying physical phenomena and corroborated with existing scientific and clinical evidence [@coffey1992quantitative]. Code available at \url{https://github.com/rohdelab/PyTransKit/blob/master/tutorials/IXI_cca.ipynb} \label{fig}](./img.pdf)

The algorithm is based on the work of [@kundu2018discovery] and provides a complete pipeline covering data preprocessing, computing the LOT embedding [@wang2013linear;@kolouri2017optimal;@basu2014detecting], and data analysis. The corresponding components include preprocessing for the calculation of LOT, the LOT computation process [@kundu2018discovery], principal component analysis (PCA) [@jolliffe2016principal], linear discriminant analysis (LDA/PLDA) [@balakrishnama1998linear;@wang2011penalized], canonical correlation analysis (CCA) [@hardoon2004canonical], and visualization of analytical results. \autoref{fig} illustrates the workflow of 3D-TBM for LOT embedding and model-based visualization.
...
Step 2 in the TBM pipeline is to compute the linearized optimal transport (LOT) embedding for each image. This is done by solving the following mathematical minimization problem [@kundu2018discovery]:
\begin{equation}
...
The open source license the software is released under the GNU General Public License (GPL), the tutorial of this project is available on \url{https://github.com/rohdelab/PyTransKit/blob/master/tutorials/17_tutorial_3DTBM.ipynb}. Over the past three months, PyTransKit has received 46 Git clones and 236 downloads; in 2025, it recorded 556 clones and 2,820 downloads.

# AI usage disclosure

Artificial intelligence (AI) tools were used for grammar checking, code comment refinement, and partial translation of code from MATLAB to Python. All AI-generated outputs were reviewed and verified by the authors, who take full responsibility for the final content.
